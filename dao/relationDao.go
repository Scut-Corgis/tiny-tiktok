...
userId 关注 followId
*/
func QueryIsFollowByUserId(userId int64, followId int64) (int64, error) {
	// isFollow为0，表示未关注，即不存在这条关系
	var isFollow int64
	err := Db.Table("follows").Where("user_id = ? and follower_id = ?", followId, userId).Count(&isFollow).Error
	return isFollow, err
}
...
// QueryFollowersByUserId 有bug，暂不启用
// /*
// 查询用户粉丝列表
// */
...
func QueryFollowersIdByUserId(userId int64) ([]int64, error) {
	...
	if err := Db.Raw("select follower_id from follows where user_id = ?", userId).Scan(&followerIds).Error; nil != err {
		return nil, err
	}
